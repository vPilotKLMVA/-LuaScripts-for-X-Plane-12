...
2. **Aircraft Mismatch Detection**
   - Compares flight plan aircraft vs actual aircraft
   - Alerts if wrong aircraft loaded
   - Uses XPLM FFI for aircraft detection
...
## 7. ✅ XPLM FFI Integration

### New Module: `xplm_ffi.lua`

### Capabilities
1. **Aircraft Detection**
   ```lua
   XPLM_FFI.get_aircraft_type()      -- Returns ICAO code
   XPLM_FFI.get_aircraft_livery()    -- Returns livery path
   XPLM_FFI.get_tail_number()        -- Returns registration
   ```

2. **Validation**
   ```lua
   XPLM_FFI.validate_aircraft(expected_icao)  -- Returns true/false
   ```

3. **Cross-Platform Support**
   - Windows: `XPLM_64.dll`
   - macOS: `XPLM.framework/XPLM`
   - Linux: `XPLM_64.so`

### Use Cases
- Offline fallback data access
- Aircraft mismatch detection
- Enhanced logging with aircraft details
- Future: Auto-populate flight plan fields
...
- **NEW**: XPLM validation: Every frame
...
### XPLM FFI
- [ ] Aircraft type detected
- [ ] Works on Windows/Mac/Linux
- [ ] Graceful failure if FFI unavailable
- [ ] Mismatch alarm triggers
...
2. **Install new version**
   ...
   - New files (`xplm_ffi.lua`, `alarms.lua`) auto-load
...
4. **XPLM FFI**
   - Requires FFI support in Lua
   - May not work on all X-Plane installations
